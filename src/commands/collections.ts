import { getCollections } from '../os/collections';
...
const findSuitableCollections = async (wallets: LoggedInWallet[]): Promise<void> => {
  const collections = await getCollections(300,wallets[0]);
  ...
  collections.forEach((collection) => {
    wallets.forEach((wallet) => {
      data.push(
        `${wallet.address};${collection.contractAddress};${collection.name};${(collection as any).collectionSlug};${
